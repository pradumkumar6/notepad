...
public class Edit {
  ...
  public void editText(String command) {
    switch (command) {
      case "Copy to Stack":
        gui.stack.push(gui.textArea.getSelectedText());
        break;

      case "Copy (Ctrl + C)":
        gui.str = gui.textArea.getSelectedText();
        break;
      ...
      case "Paste from Stack":
        gui.pos = gui.textArea.getCaretPosition(); // Get Position of Cursor
        try {
      ...
      case "Paste (Ctrl + V)":
        gui.pos = gui.textArea.getCaretPosition();
